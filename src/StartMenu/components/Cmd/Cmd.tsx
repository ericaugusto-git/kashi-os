  ...
  const echo = async () => {
      localEcho
        ...
        .then(async (input: string) => {
          ...
          const escapedInput = input.replace(/\\ /g, SPACE_PLACEHOLDER);
          ...
          switch (command) {
            ...
            case "rm": {
              ...
              const path = option ? args[1] : args[0];
              try {
                if(option){
                  console.log("deleting recursively: ", path);
                  await fileSystem.deleteRecursive(`${currentDirectory}/${path}`);
                  fileSystem.refreshFileList(currentDirectory);
                }else{
                  const result = await fileSystem.deletePath(currentDirectory, path);
                  console.log(result);
                  localEcho.println(`Removed ${path}`);
                }
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              } catch (error: any) {
                console.log("error: ", error);
                if(error?.code == "ENOTEMPTY"){
                  localEcho.println(`rm: cannot remove '${path}': Directory not empty`);
                  localEcho.println(`Tip: use -r or -rf to remove directories that are not empty`);
                }else{
                  localEcho.println(`rm: cannot remove '${path}': No such file`);
                }
              }
              break;
            }
